...
@interface LHSConnection () <STCPSocketDelegate>
...
- (BOOL)isAuthenticated {
    ...
    if ([self useDigestAccessAuthentication]) {
        ...
        NSData *temp = [[NSData alloc] initWithBase64EncodedData:[base64Credentials dataUsingEncoding:NSUTF8StringEncoding] options:kNilOptions];
...
- (void)startReadingRequest {
    ...
    [asyncSocket readDataToData:[STCPSocket CRLFData]
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength {
    ...
    for (i = 0; i < [rangeComponents count]; i++) {
        ...
        if (dashRange.location == NSNotFound) {
            ...
            UInt64 byteIndex;
            if (![NSNumber lhs_parseString:rangeComponent intoUInt64:&byteIndex])
                return NO;
    ...
    if ([LHSWebSocket isWebSocketRequest:request]) {
        ...
        if (ws == nil) {
            ...
            if ([asyncSocket delegate] == self) {
                NSLog(@"%s[%p]: WebSocket forgot to set itself as socket delegate", __FILE__, self);
            ...
            if ([data length] > 0) {
                ...
                if (isChunked) {
                    ...
                    if ([httpResponse isDone]) {
                        ...
                        NSData *footer = [STCPSocket CRLFData];
...
- (void)continueSendingStandardResponseBody {
    ...
    if (writeQueueSize >= LHSReadChunkSize)
        return;
...
- (NSString *)filePathForURI:(NSString *)path allowDirectory:(BOOL)allowDirectory {
    ...
    if (documentRoot == nil) {
        NSLog(@"%s[%p]: No configured document root", __FILE__, self);
        return nil;
    }
    ...
    if (docRoot == nil) {
        NSLog(@"%s[%p]: Document root is invalid file path", __FILE__, self);
        return nil;
    }
    ...
    if (![fullPath hasPrefix:documentRoot]) {
        NSLog(@"%s[%p]: Request for file outside document root", __FILE__, self);
        return nil;
    }
 ...
 * The data parameter is the invalid HTTP header line, including CRLF, as read from STCPSocket.
...
#pragma mark STCPSocket Delegate
...
- (void)socket:(STCPSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    if (tag == LHSHTTPRequestHeader) {
        ...
        if (!result) {
            NSLog(@"%s[%p]: Malformed request", __FILE__, self);
            ...
            if (expectsUpload) {
                if (transferEncoding && ![transferEncoding caseInsensitiveCompare:@"Chunked"]) {
                    ...
                    if (contentLength == nil) {
                        NSLog(@"%s[%p]: Method expects request body, but had no specified Content-Length",
                              __FILE__,
                              self);
                ...
                if (contentLength != nil) {
                    ...
                    if (requestContentLength > 0) {
                        NSLog(@"%s[%p]: Method not expecting request body had non-zero Content-Length",
                              __FILE__,
                              self);
        ...
        if (tag == LHSHTTPRequestChunkSize) {
            ...
            if (errno != 0) {
                NSLog(@"%s[%p]: Method expects chunk size, but received something else", __FILE__, self);
            ...
            if (![data isEqualToData:[STCPSocket CRLFData]]) {
                NSLog(@"%s[%p]: Method expects chunk trailer, but is missing", __FILE__, self);
 ...
 **/
- (void)socket:(STCPSocket *)sock didWriteDataWithTag:(long)tag {
 ...
 **/
- (void)socketDidDisconnect:(STCPSocket *)sock withError:(NSError *)err {
    // HTTPLogTrace();
...
- (void)responseHasAvailableData:(NSObject<LHSResponse> *)sender {
    ...
    dispatch_async(connectionQueue, ^{ @autoreleasepool {
        if (sender != httpResponse) {
            NSLog(@"%s[%p]: %s - Sender is not current httpResponse", __FILE__, self, __PRETTY_FUNCTION__);
            return;
        }
