...
- (void)setDocumentRoot:(NSString *)value
{
	...
	{
		NSLog(@"%s: %s - Expecting NSString parameter, received %@ parameter",
					__FILE__, __PRETTY_FUNCTION__, NSStringFromClass([value class]));
		return;
	}
...
- (BOOL)start:(NSError **)errPtr
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		{
			NSLog(@"%s: Failed to start HTTP Server: %@", __FILE__, err);
		}
	}});
