...
- (id) initWithData:(NSData *)data contentEncoding:(NSStringEncoding)encoding {
	...
	if( (-1 == separatorOffset) || (separatorOffset >= length-2) ) {
		NSLog(@"MultipartFormDataParser: Bad format.No colon in field header.");
		// tear down
		return nil;
	}
	...
	if( nil == name ) {
		NSLog(@"MultipartFormDataParser: Bad MIME header name.");
		// tear down
		return nil;		
	}
	...
	if( separatorOffset == -1 ) {
		...
		if( nil == value ) {
			NSLog(@"MultipartFormDataParser: Bad MIME header value for header name: '%@'",name);
			// tear down
			return nil;		
		}
		return self;
	...
	if( ![self parseHeaderValueBytes:bytes length:length encoding:encoding] ) {
		...
		NSLog(@"MultipartFormDataParser: Bad params for header with name '%@' and value '%@'",name,value);
		NSLog(@"MultipartFormDataParser: Params str: %@",paramsStr);
...
-(BOOL) parseHeaderValueBytes:(char*) bytes length:(NSUInteger) length encoding:(NSStringEncoding) encoding {
	...
	while( offset < length ) {
		...
		if( bytes[offset] == ';' ) {
			if( !currentParam ) {
				...
				NSLog(@"MultipartFormDataParser: Unexpected ';' when parsing header");
				return NO;
			}
			NSString* paramValue = extractParamValue(bytes, offset,encoding);
			 if( nil == paramValue ) {
				NSLog(@"MultipartFormDataParser: Failed to exctract paramValue for key %@ in header %@",currentParam,name);
			}
			else {
#ifdef DEBUG
				if( [params objectForKey:currentParam] ) {
					NSLog(@"MultipartFormDataParser: param %@ mentioned more then once in header %@",currentParam,name);
				}
#endif
	...
	// add last param
	if( insideQuote ) {
		NSLog(@"MultipartFormDataParser: unterminated quote in header %@",name);
//		return YES;
	}
	if( currentParam ) {
...
#ifdef DEBUG
		if( [params objectForKey:currentParam] ) {
			NSLog(@"MultipartFormDataParser: param %@ mentioned more then once in one header",currentParam);
		}
#endif
		[params setObject:paramValue forKey:currentParam];
		// HTTPLogVerbose(@"MultipartFormDataParser: header param: %@ = %@",currentParam,paramValue);
