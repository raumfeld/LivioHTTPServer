...
- (id) initWithBoundary:(NSString*) boundary formEncoding:(NSStringEncoding) _formEncoding {
	...
	if( nil == boundary ) {
		NSLog(@"MultipartFormDataParser: init with zero boundary");
		return nil;
	}
...
- (BOOL) appendData:(NSData *)data { 
    ...
    if( nil == boundaryData ) {
		NSLog(@"MultipartFormDataParser: Trying to parse multipart without specifying a valid boundary");
		assert(false);
        return NO;
    }
		...
		if( nil == currentHeader ) {
			...
			if( -1 == headerEnd ) {
				...
				if( nil == currentHeader ) {
					...
					NSLog(@"MultipartFormDataParser: MultipartFormDataParser: wrong input format, coulnd't get a valid header");
					return NO;
				}
...
- (int) offsetTillNewlineSinceOffset:(int) offset inData:(NSData*) data {
	...
	while ( *(uint16_t*)(bytes + offset) != 0x0A0D ) {
		...
		if( !isspace(*(bytes+offset)) ) {
			NSLog(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset) );
		}
		if( !isspace(*(bytes+offset+1)) ) {
			NSLog(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset+1) );
		}
#endif
		offset++;
		if( offset >= length ) {
...
//	http://tools.ietf.org/html/rfc2045#section-6.7
	...
	while( count < length ) {
		if( bytes[count] == '=' ) {
			...
			if( length < 3 ) {
				NSLog(@"MultipartFormDataParser: warning, trailing '=' in quoted printable data");
			}
			// soft newline
			if( bytes[0] == '\r' ) {
				bytes += 1;
		...
		if( (unsigned char) bytes[count] > 126 ) {
			NSLog(@"MultipartFormDataParser: Warning, character with code above 126 appears in quoted printable encoded data");
		}
#endif
